...
void eepromInit(boolean initialWrite) {
  ...
  if (initialWrite) {
    ...
    EEPROM.write(5, 0); //temp alert threshold - 100 deg
    EEPROM.write(6, 0); //temp critical threshold - 110 deg
...
float getVolts(){
  ...
  float toReturn = (float)result / 10.0; //divide by 10 and convert back to float to get the single decimal place back

  if (toReturn < 0) { return 0.0; }
  else {
    return toReturn;
  }
...
static int tempAlarmInterval = 500;
boolean freqFlip = false;
int alertCounter = 0;
unsigned int long voltsTime;
static int voltAlarmInterval = 8000;
unsigned int long alertSilencedTime = 0;
void alarms() {

  //if alerts were silenced
  if (alertSilenced) {
    if (alertSilencedTime == 0) {
      alertSilencedTime = curTime;
    }
    else if (curTime - alertSilencedTime > 1000 * 60) {
      alertSilenced = false;
      alertSilencedTime = 0;
    }
    return;
  ...
  //temp alarms
  if (tempAlertThreshold > 0 && curTime - tempTime > tempAlarmInterval){

    tempTime = curTime;
    
    //critical temp
    if (tempCriticalThreshold > 0 && getTemp() > tempCriticalThreshold) {
      if (freqFlip) {
        tone(buzzPin, 4000, 500);
      }
      else {
        tone(buzzPin, 4500, 500);
      }
      freqFlip = !freqFlip;
    ...
    //alert temp
    else if (getTemp() > tempAlertThreshold) {
      if (alertCounter == 0) {
  ...
  //volt alarms
  if (voltAlarmMode > 0 && curTime - voltsTime > voltAlarmInterval) {

    voltsTime = curTime;
    
    //low battery
    if (voltAlarmMode == 1 && getVolts() < 11.5) {
      tone(buzzPin, 4000, 200);
    }
    //not charging
    if ((voltAlarmMode == 2 || voltAlarmMode == 4) && getVolts() < 12.5) {
      tone(buzzPin, 4000, 800);
    }
    //over charging
    if ((voltAlarmMode == 3 || voltAlarmMode == 4) && getVolts() > 15) {
      tone(buzzPin, 4000, 2200);
    }
...
//update the LCD
#define lcdDemo true
int static lcdRefreshRate = 180;
unsigned int long lcdTimer;
boolean lcdRunOnce = false;
void lcdDraw() {
  ...
  if (!lcdRunOnce) {
    ...
    oled.drawString(10, 4, "GCU rev 1", PALETURQUOISE, BLACK);
    oled.selectFont(Droid_Sans_12);
    oled.drawString(4, 110, "A firmware upgrade is\nrequired to use LCD!", ORANGERED, BLACK);
     oled.drawString(4, 40, "www.dangerflite.com.au", RED, BLACK);
  ...
  if (lcdDemo) {
    ...
    oled.drawString(4, 80, "Volts: " + (String)getVolts() + "v    ", LIMEGREEN, BLACK);
    oled.drawString(4, 60, "Temp: " + (String)getTemp() + "degC   ", SKYBLUE, BLACK);
